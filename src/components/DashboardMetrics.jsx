...
export default function DashboardMetrics({ companyTasks, language }) {
  return (
    <div className="metrics-grid">
      <div className="metric-card animate-fade-in" style={{animationDelay: '0.1s'}}>
        <div className="metric-title">{language === 'id' ? 'Pesanan Aktif' : 'Active Orders'} <Activity size={16} /></div>
        <div className="metric-value">{companyTasks.length}</div>
      </div>
      <div className="metric-card animate-fade-in" style={{animationDelay: '0.2s'}}>
        <div className="metric-title">{language === 'id' ? 'Fase Perencanaan' : 'Planning Phase'} <Search size={16} /></div>
      ...
      <div className="metric-card animate-fade-in" style={{animationDelay: '0.3s'}}>
        <div className="metric-title">{language === 'id' ? 'Fase Pengiriman' : 'Delivery Phase'} <ShoppingCart size={16} /></div>
